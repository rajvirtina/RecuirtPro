...
const SESSION_VIOLATION_MAP = {
    tab_switch: types_1.ProctoringEventType.TAB_SWITCH,
    window_blur: types_1.ProctoringEventType.WINDOW_BLUR,
    multiple_faces: types_1.ProctoringEventType.MULTIPLE_FACES,
    no_face: types_1.ProctoringEventType.NO_FACE_DETECTED,
    copy_attempt: types_1.ProctoringEventType.SUSPICIOUS_BEHAVIOR,
};
const SESSION_VIOLATION_DESC = {
...
async function resolveSessionOrFail(sessionId, res) {
    ...
    if (session.status === 'expired' || new Date() > session.expiresAt) {
        ...
        (0, response_1.sendError)(res, 'Session has expired', 410);
        return null;
    }
    return session;
}
...
const recordConsent = async (req, res) => {
    try {
        ...
        const candidate = await models_2.User.findById(session.candidateId)
        ...
        await AIInterviewSession_1.AIInterviewSession.updateOne({ sessionId }, { consentGiven: Boolean(consented), consentGivenAt: new Date() });
        // Log a proctoring event so HR can see consent was captured
        await models_1.ProctoringEvent.create({
            interviewId: session.interviewId,
            candidateId: session.candidateId,
            eventType: consented ? types_1.ProctoringEventType.CONSENT_GIVEN : types_1.ProctoringEventType.CONSENT_DENIED,
            severity: 'low',
        ...
        return (0, response_1.sendSuccess)(res, { consented }, 'Consent recorded');
    }
    catch (error) {
        logger_1.default.error('recordConsent error:', error);
        return (0, response_1.sendError)(res, error.message || 'Failed to record consent', 500);
    }
};
 ...
 * @auth  None (session token is the credential)
 */
const logSessionViolation = async (req, res) => {
    try {
        const { sessionId } = req.params;
        ...
        if (!session)
            return;
        const eventType = SESSION_VIOLATION_MAP[type] ?? types_1.ProctoringEventType.VIOLATION;
        ...
        const event = await models_1.ProctoringEvent.create({
            ...
            snapshotUrl: screenshotBase64 && screenshotBase64.length < 150000
                ? screenshotBase64
                : undefined,
            metadata: {
        ...
        (0, socketController_1.emitViolation)(session.interviewId.toString(), event);
        return (0, response_1.sendSuccess)(res, { logged: true, eventId: event._id }, 'Violation logged');
    }
    catch (error) {
        logger_1.default.error('logSessionViolation error:', error);
        return (0, response_1.sendError)(res, 'Failed to log violation', 500);
    }
};
exports.logSessionViolation = logSessionViolation;
/**
 ...
 */
const getProctoringReportByApplication = async (req, res) => {
    try {
        const { applicationId } = req.params;
        const tenantId = req.user?.companyId;
        // Find the interview(s) linked to this application
        const interview = await models_1.Interview.findOne({ applicationId })
            .select('_id companyId proctoringEnabled status scheduledTime duration candidateId')
            .lean();
        if (!interview) {
            return (0, response_1.sendSuccess)(res, { hasData: false, events: [], summary: null }, 'No interview found for this application');
        }
        // Tenant isolation
        if (tenantId && interview.companyId?.toString() !== tenantId) {
        ...
        const events = await models_1.ProctoringEvent.find({ interviewId: interview._id })
            .sort({ timestamp: 1 })
            .lean();
        ...
        const riskLevel = total >= 6 ? 'HIGH' : total >= 3 ? 'MEDIUM' : 'LOW';
        const bySeverity = {
        ...
        logger_1.default.error('getProctoringReportByApplication error:', error);
        return (0, response_1.sendError)(res, error.message || 'Failed to retrieve proctoring report', 500);
    }
};
exports.getProctoringReportByApplication = getProctoringReportByApplication;
