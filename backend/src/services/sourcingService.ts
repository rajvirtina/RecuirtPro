...
class LinkedInService {
  ...
  async searchCandidates(accessToken: string, criteria: SourcingCriteria): Promise<CandidateProfile[]> {
    if (!this.clientId) {
      logger.info('LinkedIn API not configured — returning simulated results');
    ...
    try {
      ...
      logger.warn('LinkedIn API error, falling back to simulated results:', error.message);
      return this.getSimulatedResults(criteria);
    }
  }
...
class NaukriService {
  ...
  async searchCandidates(apiKey: string | null, criteria: SourcingCriteria): Promise<CandidateProfile[]> {
    const key = apiKey || this.apiKey;
    if (!key) {
      logger.info('Naukri API not configured — returning simulated results');
      return this.getSimulatedResults(criteria);
    }

    try {
      const response = await axios.post('https://rms.naukri.com/api/resume-search', {
        keywords: criteria.keywords?.join(' '),
      ...
      logger.warn('Naukri API error, falling back to simulated results:', error.message);
      return this.getSimulatedResults(criteria);
    }
  }
...
class GitHubService {
  ...
  async searchDevelopers(accessToken: string | null, criteria: SourcingCriteria): Promise<CandidateProfile[]> {
    const token = accessToken || process.env.GITHUB_TOKEN;
    if (!token && !this.clientId) {
      logger.info('GitHub API not configured — returning simulated results');
      return this.getSimulatedResults(criteria);
    }
    ...
    try {
      ...
      if (criteria.skills?.length) queryParts.push(criteria.skills.join('+'));
      if (criteria.location) queryParts.push(`location:${criteria.location.split(',')[0].trim()}`);
      ...
      const candidates = await Promise.all(
        response.data.items.slice(0, 10).map((user: any) =>
          this.enrichGitHubUser(user, token, criteria)
        )
      );
      ...
      logger.warn('GitHub API error, falling back to simulated results:', error.message);
      return this.getSimulatedResults(criteria);
    }
  }

  private async enrichGitHubUser(user: any, token: string | null, criteria: SourcingCriteria): Promise<CandidateProfile | null> {
    try {
      const headers: Record<string, string> = { Accept: 'application/vnd.github+json' };
      if (token) headers.Authorization = `Bearer ${token}`;

      // Fetch user profile
      const profileRes = await axios.get(`https://api.github.com/users/${user.login}`, { headers });
      ...
      logger.warn(`Failed to enrich GitHub user ${user.login}`);
      return null;
    }
  }

  private calculateTechStackScore(userLangs: string[], requiredSkills: string[]): number {
    if (!requiredSkills.length) return 50;
    const matchedCount = requiredSkills.filter(skill =>
      userLangs.some(lang => lang.toLowerCase() === skill.toLowerCase())
    ).length;
...
export class AIMatchingEngine {
  ...
  static async calculateMatch(
    ...
    const matchedSkills = allRequired.filter(s =>
      candidateSkills.some(cs => cs.toLowerCase().includes(s.toLowerCase()) || s.toLowerCase().includes(cs.toLowerCase()))
    );
    const missingSkills = allRequired.filter(s =>
      !candidateSkills.some(cs => cs.toLowerCase().includes(s.toLowerCase()) || s.toLowerCase().includes(cs.toLowerCase()))
    );

    const strengths: string[] = [];
    if (skillScore >= 80) strengths.push(`Strong skill match (${matchedSkills.length}/${allRequired.length || 1} matched)`);
    if (experienceScore >= 80) strengths.push('Experience well-aligned');
    if (locationScore >= 80) strengths.push('Location match');
    if (semanticScore >= 80) strengths.push('High semantic relevance');
    if (techStack.length > 5) strengths.push('Diverse tech stack');
   ...
   */
  private static calcSkillScore(candidateSkills: string[], requiredSkills: string[], techStack: string[]): number {
    const allRequired = [...requiredSkills, ...techStack];
    if (allRequired.length === 0) return 70; // no requirements = neutral
    ...
    for (const required of allRequired) {
      ...
      const isMatch = candidateSkills.some(cs => {
        ...
        if (csLower === reqLower || csLower.includes(reqLower) || reqLower.includes(csLower)) return true;
        // Check synonyms
        for (const [key, syns] of Object.entries(synonyms)) {
          const allForms = [key, ...syns];
          if (allForms.includes(reqLower) && allForms.some(f => csLower.includes(f) || f.includes(csLower))) return true;
        }
        return false;
      });
  ...
  private static calcExperienceScore(candidateExp: number, minExp?: number, maxExp?: number): number {
    if (minExp === undefined && maxExp === undefined) return 75;
    if (candidateExp === 0) return 50; // unknown experience
  ...
  private static calcLocationScore(candidateLocation: string, requiredLocation?: string): number {
    ...
    if (cl.includes(rl) || rl.includes(cl)) return 100;

    // City-level match
    const candidateCity = cl.split(',')[0].trim();
    const requiredCity = rl.split(',')[0].trim();
    if (candidateCity === requiredCity) return 100;

    // Same country
    const candidateCountry = cl.split(',').pop()?.trim();
    const requiredCountry = rl.split(',').pop()?.trim();
    if (candidateCountry && requiredCountry && candidateCountry === requiredCountry) return 60;
...
export class SourcingService {
  ...
  private naukriService = new NaukriService();
  ...
  async exchangeOAuthCode(platform: SourcingPlatform, code: string): Promise<{ accessToken: string; expiresIn?: number; refreshToken?: string }> {
    switch (platform) {
      case SourcingPlatform.LINKEDIN:
  ...
  async searchCandidates(
    ...
    const promises = platforms.map(async (platform) => {
      try {
        ...
        logger.error(`Error sourcing from ${platform}:`, error.message);
        return [];
      }
    });
