 ...
 */
export class LinkedInSourcingService {
   ...
   */
  async searchCandidates(criteria: SourcingCriteria): Promise<CandidateProfile[]> {
    try {
      if (!this.apiKey) {
      ...
      logger.error('LinkedIn sourcing error:', error);
   ...
   */
  private getMockLinkedInCandidates(criteria: SourcingCriteria): CandidateProfile[] {
 ...
 */
export class NaukriSourcingService {
  private apiKey: string;
  private apiUrl: string;
  ...
  async searchCandidates(criteria: SourcingCriteria): Promise<CandidateProfile[]> {
    try {
      if (!this.apiKey) {
        logger.warn('Naukri API key not configured');
      ...
      logger.error('Naukri sourcing error:', error);
   ...
   */
  private getMockNaukriCandidates(criteria: SourcingCriteria): CandidateProfile[] {
    const mockCandidates: CandidateProfile[] = [
      {
        source: 'naukri',
   ...
   */
  async parseResume(resumeUrl: string): Promise<Partial<CandidateProfile>> {
    try {
      // For production: Use Naukri's resume parsing API or third-party services
      // like RChilli, Sovren, Textkernel
      logger.info(`Parsing resume from: ${resumeUrl}`);
...
export class CandidateSourcingService {
  private linkedInService: LinkedInSourcingService;
  private naukriService: NaukriSourcingService;
  ...
  async searchCandidates(
    ...
    try {
      ...
      logger.error('Candidate sourcing error:', error);
      throw error;
    }
  }
  ...
  calculateMatchScore(candidate: CandidateProfile, jobRequirements: {
    ...
    if (candidate.experience) {
      if (
        candidate.experience >= jobRequirements.experienceMin &&
        candidate.experience <= jobRequirements.experienceMax
      ) {
        score += 25;
      } else {
        const diff = Math.min(
          Math.abs(candidate.experience - jobRequirements.experienceMin),
          Math.abs(candidate.experience - jobRequirements.experienceMax)
        );
    ...
    if (jobRequirements.location && candidate.location) {
      if (
        candidate.location.toLowerCase().includes(jobRequirements.location.toLowerCase()) ||
